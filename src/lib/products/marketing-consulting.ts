...
export const MC_SCOPE_CLAUSE = 'Marketing Consulting is advisory. The Practice provides strategy, research, and recommendations and does not execute on them. The engagement includes only what is set for the chosen level in this Schedule: the monthly direct phone access, the scheduled strategy call, the number of research advisories, the reporting, and, where included, operations consulting. The monthly fee secures the Practice\'s attention and thinking for that defined cadence, not unlimited production. A request beyond what the chosen level includes, such as additional research advisories or additional access, is new work that the Practice scopes and quotes before any work begins under the change-order mechanism in section 8. Executing a recommendation is not consulting and does not run through a change order; per section 3.2 it routes to Web Management hours when the work fits an existing managed site and the pool has room, to Web Management overage with the Client\'s written approval, to a Build Statement of Work, or to a separate Statement of Work for non-web execution.';
